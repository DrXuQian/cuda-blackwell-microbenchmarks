...
int main() {
     ...
     */
    // Use the most compatible CUTLASS configuration that works on RTX 5070
    using ThreadblockShape = cutlass::gemm::GemmShape<128, 128, 8>;
    using WarpShape = cutlass::gemm::GemmShape<32, 64, 8>;
    using InstructionShape = cutlass::gemm::GemmShape<1, 1, 1>;
    
    using CutlassGemm = cutlass::gemm::device::Gemm<
        float,                  // ElementA: Data type of matrix A (FP32)
        cutlass::layout::RowMajor,  // LayoutA: Memory layout of matrix A
        float,                  // ElementB: Data type of matrix B (FP32)  
        cutlass::layout::RowMajor,  // LayoutB: Memory layout of matrix B
        float,                  // ElementC: Data type of matrix C (FP32)
        ...
        cutlass::arch::OpClassSimt,  // Operator class: Use CUDA Cores (more compatible)
        cutlass::arch::Sm50,    // Architecture: SM50 for maximum compatibility
    ...
    cutlass::HostTensor<float, cutlass::layout::RowMajor> tensor_D({M, N}); // Output
    ...
    cutlass::HostTensor<float, cutlass::layout::RowMajor> reference({M, N});
    ...
    CUDA_CHECK(cudaMemcpy(reference.device_data(), tensor_C.device_data(), 
                         M * N * sizeof(float), cudaMemcpyDeviceToDevice));

...
// Load functor for LayerNorm input
template<typename T, typename ComputeType>
struct AffineLoad {
    using LoadType = T;
    AffineLoad(const T* x, const ComputeType* gamma, const ComputeType* beta, int64_t cols)
        : x(x), gamma(gamma), beta(beta), cols(cols) {}

    template<int N>
    __device__ void load(T* dst, int64_t row, int64_t col) const {
        for (int i = 0; i < N; i++) {
            dst[i] = x[row * cols + col + i];
        }
    }
...
// Store functor for LayerNorm output
template<typename T, typename ComputeType>
struct AffineStore {
    AffineStore(T* y, const ComputeType* gamma, const ComputeType* beta, int64_t cols)
        : y(y), gamma(gamma), beta(beta), cols(cols) {}

    template<int N>
    __device__ void store(const ComputeType* src, int64_t row, int64_t col) {
        for (int i = 0; i < N; i++) {
            y[row * cols + col + i] = static_cast<T>(src[i] * gamma[col + i] + beta[col + i]);
        }
    }
...
template<typename T>
void cpu_layer_norm(const T* input, T* output, const float* gamma, const float* beta,
                    int batch, int embed, float epsilon) {
    for (int b = 0; b < batch; b++) {
        ...
        for (int i = 0; i < embed; i++) {
            float normalized = (static_cast<float>(x[i]) - mean) * inv_std;
            y[i] = static_cast<T>(normalized * gamma[i] + beta[i]);
        }
    }
}
...
void run_test(int batch, int embed) {
    ...
    float* h_gamma = new float[embed];
    float* h_beta = new float[embed];
    ...
    T *d_input, *d_output;
    float *d_gamma, *d_beta, *d_mean, *d_inv_var;
    cudaMalloc(&d_input, size * sizeof(T));
    cudaMalloc(&d_output, size * sizeof(T));
    cudaMalloc(&d_gamma, embed * sizeof(float));
    cudaMalloc(&d_beta, embed * sizeof(float));
    cudaMalloc(&d_mean, batch * sizeof(float));
    cudaMalloc(&d_inv_var, batch * sizeof(float));

    // Copy data to device
    cudaMemcpy(d_input, h_input, size * sizeof(T), cudaMemcpyHostToDevice);
    cudaMemcpy(d_gamma, h_gamma, embed * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_beta, h_beta, embed * sizeof(float), cudaMemcpyHostToDevice);

    // Run GPU kernel
    using ComputeType = float;
    AffineLoad<T, ComputeType> load(d_input, d_gamma, d_beta, embed);
    AffineStore<T, ComputeType> store(d_output, d_gamma, d_beta, embed);

    DispatchLayerNormWarpImpl<decltype(load), decltype(store), ComputeType>(
        cudaStreamDefault, load, store, batch, embed, epsilon, d_mean, d_inv_var);
    ...
    cpu_layer_norm(h_input, h_output_ref, h_gamma, h_beta, batch, embed, epsilon);
    ...
    std::cout << "LayerNorm Test - Batch: " << batch << ", Embed: " << embed << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "Running LayerNorm with:" << std::endl;
